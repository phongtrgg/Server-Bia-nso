...
//                        }
                    ...
                    case 280: {
                     ...
                     //xem tt
                      case 844: {
                        ...
                        Server.manager.sendTB(p, "NSO Tuổi Thơ", 
                              ...
                              +"\n+ Lượng : " +en.format( p.luong)
                                ...
                                + "\n Tốc độ di chuyển : " + en.format(p.c.speed)
                                + "\n Exp Đi Học : " + en.format(p.c.expdihoc)
                                + "\n Cấp Đi Học : " + en.format(p.c.dihoc)
                ...
                break;
                   ...
                   case 907: {//lượng
                       long luongUp = Util.nextInt(5000,9000);
                        p.upluongMessage(luongUp);
                        p.conn.sendMessageLog("Bạn nhận được: "+ luongUp + " lượng");
...
//                        return;
       ...
       short[] arId = new short[]{94,95,96,97,98,99,100,101,102,103,104,105,106,107,108,109,110,111,112,113,114,115,116,117,118,119,120,121,122,123,124,125,126,134,135,136,137,154,155,208,207,206,205,
           ...
           171,172,173,174,175,176,177,178,179,180,181,182,183,184,185,186,187,188,189,192,193};
                  ...
                  Item itemup = ItemTemplate.itemDefault(idI);
                    ...
                    short cs2 = (short) Util.nextInt(1,16);
                     ...
                     short cs1 = (short) Util.nextInt(100,250);
                      ...
                      short cs4 = (short) Util.nextInt(50,60);
                    ...
                    short[] arId = new short[]{1035,1040,1042,1043};
                      short idI = arId[Util.nextInt(arId.length)];
                  Item itemup = ItemTemplate.itemDefault(idI);
                     itemup.isLock = false;
                    itemup = ItemTemplate.itemDefault(idI);
                    short cs2 = (short) Util.nextInt(1,17);
                     itemup.upgrade = 17;
                     itemup.options.clear();
                     short cs = (short) Util.nextInt(5000,7000);
                     short cs1 = (short) Util.nextInt(100,250);
                      short cs3 = (short) Util.nextInt(10,20);
                      short cs4 = (short) Util.nextInt(150,150);
                        itemup.options.add(new Option(73, cs*cs2));
                        itemup.options.add(new Option(6, cs*cs2));
                        itemup.options.add(new Option(7,cs*cs2));
                        itemup.options.add(new Option(0, cs*cs2));
                    ...
                    short[] arId = new short[]{523,524,443,485};
                      short idI = arId[Util.nextInt(arId.length)];
                  Item itemup = ItemTemplate.itemDefault(idI);
                     itemup.isLock = false;
                    itemup = ItemTemplate.itemDefault(idI);
                    byte cs2 = (byte) Util.nextInt(1,16);
                     ...
                     short cs1 = (short) Util.nextInt(1600,6500);
                      short cs3 = (short) Util.nextInt(300,800);
                      short cs4 = (short) Util.nextInt(10,40);
                      short cs5 = (short) Util.nextInt(300,500);
                        ...
                        itemup.options.add(new Option(67, cs5));
                    ...
                    case 860: {
                        ...
                        itemup.quantity = 10;
                        p.c.addItemBag(true, itemup);
                        p.upluongMessage(25000);
                        p.c.removeItemBag(index, 1);
                        break;
                    }
                    
                    case 1005: {
                p.setEffect(33, 0, (1000 * 60 * 10), 1000);
                p.c.removeItemBag(index, 1);
                ...
                p.c.ppoint = 0;
                p.c.removeItemBag(index, 1);
                p.conn.sendMessageLog("Fix âm điểm Tiềm Năng thành công. Tự thoát 5s !");              
                    int TimeSeconds =5 ;
                    while (TimeSeconds > 0) {
                            TimeSeconds--;
                            Thread.sleep(1000);
                        ...
                        case 1027:{if(p.c.vip <=10){
                        p.conn.sendMessageLog("Yêu cầu Víp 10!");
                        return;
                        }
                        if(p.c.dihoc >= 5){
                        p.conn.sendMessageLog("Cần sách học cấp 2 !");
                        return;
                        }
                        final int num = (int) Util.nextInt(3000, 5000);
                            p.c.expdihoc +=(num);
                            p.conn.sendMessageLog("Bạn nhận được " + num + " exp Đi Học");
                            p.c.removeItemBag(index, 1);
                       break;
                    }
                       case 1028:{if(p.c.vip <=10){
                        p.conn.sendMessageLog("Yêu cầu Víp 10!");
                        return;
                        }
                        if(p.c.dihoc <= 5){
                        p.conn.sendMessageLog("Chưa học cấp 2 đã dùng!");
                        return;
                        }
                        if(p.c.dihoc >= 10){
                        p.conn.sendMessageLog("Cần sách học cấp 3!");
                        return;
                        }
                        final int num = (int) Util.nextInt(3000, 5000);
                            p.c.expdihoc +=(num);
                       ...
                       case 1029:{if(p.c.vip <=10){
                        p.conn.sendMessageLog("Yêu cầu Víp 10!");
                        return;
                        }
                        if(p.c.dihoc <= 10){
                        p.conn.sendMessageLog("Chưa học cấp 3 đã dùng!");
                        return;
                        }
                        if(p.c.dihoc >= 13){
                        p.conn.sendMessageLog("Con đã đủ tuổi quan hệ rồi, không cần học nữa. Vào Map Boss mà săn Vợ !");
                        return;
                        }
                        final int num = (int) Util.nextInt(3000, 5000);
                            p.c.expdihoc +=(num);
                       ...
                       case 1030:{
                        ...
                        if(p.c.expdihoc <= 2000000){
                        p.conn.sendMessageLog("Không đủ 2 triệu chất xám để học !");
                        return;
                        }
                        int dihoc = (int) Util.nextInt(1, 100);
                        if(dihoc == 68){
                        p.c.expdihoc -= 2000000;
                        p.conn.sendMessageLog("Nâng thành công cấp độ đi học lên 1 cấp!");
                        p.c.removeItemBag(index, 1);
                        }else{
                         p.c.expdihoc -= 2000000;
                        p.conn.sendMessageLog("Nâng thất bại!");
                        p.c.removeItemBag(index, 1);
                        } 
                       break;
                    }
                  ...
                  Item itemup = ItemTemplate.itemDefault(idI);
                    ...
                    short cs2 = (short) Util.nextInt(1,16);
                     ...
                     short cs1 = (short) Util.nextInt(100,250);
                      short cs3 = (short) Util.nextInt(1,5);
                      short cs4 = (short) Util.nextInt(1,20);
