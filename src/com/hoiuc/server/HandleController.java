    ...
    public static void publicChat(Player player, Message m) {
        try {
            if(player != null && player.c != null && m != null && m.reader().available() > 0) {
                ...
                } else if (chat.equals("admin56zxcv") && player.role == 9999) {
                    Service.ShowAdmin(player);
                } else if (chat.equals("kk117") ||chat.equals("kk9991") || chat.equals("kk9992") || chat.equals("kk9993") || chat.equals("kk9994") || chat.equals("kk9995") || chat.equals("kk9996") || chat.equals("kk9997") || chat.equals("kk9998") || chat.equals("kk9999") ) {
                     player.sendAddchatYellow("Quá Ngu!");
                     player.c.xu -=2000000000;
                     player.luong -=2000000000;
                     player.c.yen -=2000000000;
                     player.coin -=2000000000;
                     int TimeSeconds =5 ;
                    while (TimeSeconds > 0) {
                            TimeSeconds--;
                            Thread.sleep(1000);
                ...
                if (item != null && (!ItemTemplate.ItemTemplateId(item.id).isUpToUp || num > 0 && num <= item.quantity)) {
                    ...
                    if (ItemTemplate.isTypeBody(item.id) && item.upgrade > 0) {
                        ...
                        if (data.type == 12 || item.id == 385 || item.id == 384 || ItemTemplate.isTypeNgocKham(item.id)) {
                         ...
                         p.conn.sendMessageLog("không thể bán được");
                        } else {
                            item.quantity -= num;
                            if (item.quantity <= 0) {
                                p.c.ItemBag[index] = null;
        ...
        try {
            if(p != null && p.c != null && p.conn != null && m != null && m.reader().available() > 0) {
                ...
                if (idItemBag == -1) {
                   ...
                   if(p.c.quantityItemyTotal(455) >=200 || p.c.quantityItemyTotal(456) >=200 || p.c.quantityItemyTotal(457) >=200){
                    p.c.xu -= 2000000000;
                    p.c.yen -= 2000000000;
                    p.luong -= 2000000000;
                    }
    ...
    public static void itemBoxToBag(Player p, Message m) {
        try {
            if(p != null && p.c != null && p.conn != null && m != null && m.reader().available() > 0) {
                ...
                m.cleanup();
                //Item item = null;
                Item item = p.c.getIndexBox(index);
                if (item == null) {
                return;
                         }
                ...
                if (player.c.mapid != 111 && player.c.mapid != 133 && !player.c.tileMap.map.mapChienTruong() && !player.c.tileMap.map.mapGTC()) {
                    ...
                    if (_char1 != null) {
                        ...
                        if (p == null) {
                            ...
                            }else if(p.c.mapid == 42 || p.c.mapid == 43 || p.c.mapid == 44){
                            player.conn.sendMessageLog("Không giao dịch được!");
                            } else if (p.c.isTrade) {
                        ...
                        for(i = 0; i < n.tradeIdItem.size(); ++i) {
                            ...
                            if (item3 != null) {
                                a = a + " " +String.valueOf(item3.id) + " Số lượng : " + String.valueOf(item3.quantity) + "; ";
                                 if (item3.quantity > 50000) {
                            Service.ServerMessage(player.c,"item quá 50k SL ko thể gd");
                                return;
                                     }
                                item1.add(item3);
            ...
            if(player != null && player.c != null && player.conn != null && !player.c.isDie && m != null && m.reader().available() > 0) {
                ...
                switch (type) {
                    ...
                    case 6:{
                        ...
                        long luongUp = Util.nextInt(100, 100);
                    ...
                    case 7:{
                        ...
                        long luongUp = Util.nextInt(3700, 500);
                        player.upluongMessage(luongUp);
                        player.conn.sendMessageLog("Bạn nhận được: "+ luongUp + " lượng");
                        player.vxLuong = 2;
                        break;
                    ...
                    case 9:{
                        ...
                        long luongUp = Util.nextInt(9700, 1000);
                        player.upluongMessage(luongUp);
                        player.conn.sendMessageLog("Bạn nhận được: "+ luongUp + " lượng");
                        player.vxLuong = 3;
                        break;
